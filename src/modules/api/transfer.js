...
export const getAllUser = (token, page, search) => {
  const URL = !search
    ? `${process.env.NEXT_PUBLIC_BACKEND_URL}/user?limit=4&sort=firstName%20ASC&page=${page}`
    : `${process.env.NEXT_PUBLIC_BACKEND_URL}/user?limit=4&search=${search}&sort=firstName%20ASC&page=${page}`;
